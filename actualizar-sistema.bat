...
:: 4. Reinstalar dependencias si cambiaron los package.json
...
:: 5. Actualizar repo de almacen (lacasitadeli-almacen)
set ALMACEN=%~dp0..\lacasitadeli-almacen
if exist "%ALMACEN%\.git" (
    echo [%date% %time%] Revisando actualizaciones en repo almacen... >> "%LOG%"
    cd /d "%ALMACEN%"

    git fetch origin main >> "%LOG%" 2>&1
    if errorlevel 1 (
        echo [%date% %time%] AVISO: No se pudo conectar a GitHub para almacen. Continuando sin actualizar almacen. >> "%LOG%"
    ) else (
    ...
    echo [%date% %time%] AVISO: No se encontro repo almacen en %ALMACEN%. Clonando... >> "%LOG%"
    cd /d "%~dp0.."
    git clone https://github.com/Konekt-ai/lacasitadeli-almacen >> "%LOG%" 2>&1
    cd /d "%~dp0"
)

:: 6. Reconstruir bodega si existe
set BODEGA=%~dp0..\lacasitadeli-almacen\pwa-bodega
if exist "%BODEGA%\package.json" (
